...
const EditBooks = () => {
  const {id} = useParams();
  const {bookTitle, authorName, category, price, bookDescription, bookpdfurl, imageurl} = useLoaderData();
  ...
  const handleBookUpdate = (e) => {
    ...
    // update book data
    fetch(`http://localhost:4044/update-book/${id}`
      ,{
        method: 'PUT',
        headers: {
...
imageurl" value="Book Image URL" />
            ...
            <TextInput id="
imageurl" name='imageurl' defaultValue={imageurl} type="text" placeholder="
...
Book Description" />
          ...
          <Textarea className='w-full' name="bookDescription" defaultValue={bookDescription} id="
